...
from a1z.config import get_control_defaults
from a1z.robots.trajectory import RecordingSession, Trajectory, play_trajectory_blocking
...
class IsaacSimArmRobot:
    ...
    def _start_impl(
        ...
        if not articulation.is_valid():
            raise RuntimeError(f"Invalid articulation root: {self._articulation_root_prim}")
        ...
        with self._command_lock:
            self._command.pos = self._full_pos[self._arm_joint_indices].copy()
    ...
    def get_robot_info(self) -> Dict[str, Any]:
        with self._state_lock:
            joint_limits = None if self._joint_limits is None else self._joint_limits.copy()
        actual_kp = None
        actual_kd = None
        effort_modes = None
    ...
    def move_joints(
        ...
        max_dist = float(np.max(np.abs(arm_target - current_arm)))
        ...
        wait_timeout_s = max(5.0, (max_dist / speed) + 5.0)
    ...
    def _set_joint_state_now(
        ...
        with self._command_lock:
            ...
            self._command.torque_ff = eff.copy()
        self._trajectory = None
        self._apply_control_action()
    ...
    def _start_trajectory(
        ...
        duration_s = max(self._control_period_s, max_dist / speed)
    ...
    def _apply_control_action(self) -> None:
        ...
        with self._command_lock:
            pos_target = self._command.pos.copy()
            vel_target = self._command.vel.copy()
        ...
        if self._with_gripper and self._gripper_joint_indices.size == 2:
            grip = self._normalized_to_gripper_dofs(self._gripper_target_value)
            self._controller().apply_action(
                ArticulationAction(
    ...
    def _clip_arm_pos(self, pos: np.ndarray) -> np.ndarray:
        ...
        if self._joint_limits is None:
            return clipped
        for local_idx, dof_idx in enumerate(self._arm_joint_indices):

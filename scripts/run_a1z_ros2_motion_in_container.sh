...
if [[ "$ACTION" == "stop" || "$ACTION" == "restart" || "$ACTION" == "status" ]]; then
  docker exec "$ROS_CONTAINER_NAME" bash -lc "
    ...
    ps -eo pid=,args= | grep -E 'a1z_motion.launch.py|a1z_motion/robot_state|a1z_motion/motion_executor|a1z_d405/d405_bridge' | grep -v grep
  "
...
docker exec \
  ...
  bash -lc '
    ...
    if [[ "'"$ACTION"'" == "start" || "'"$ACTION"'" == "restart" ]]; then
      : > "'"$RUN_LOG_PATH"'"
      nohup ros2 launch a1z_motion a1z_motion.launch.py >"'"$RUN_LOG_PATH"'" 2>&1 &
      echo "$!" >"'"$RUN_PID_PATH"'"
      disown || true
      exit 0
    fi
    ...
    exec ros2 launch a1z_motion a1z_motion.launch.py
  '

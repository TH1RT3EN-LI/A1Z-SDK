...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Capture one frame from the profile-selected ROS RGB-D contract."
    )
    parser.add_argument(
        "--color-topic",
        default=os.environ.get("A1Z_RGBD_COLOR_TOPIC", "/a1z/d405/color/image_raw"),
    )
    parser.add_argument(
        "--depth-topic",
        default=os.environ.get("A1Z_RGBD_DEPTH_TOPIC", "/a1z/d405/depth/image_rect"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target-frame-id", default=os.environ.get("A1Z_RGBD_TARGET_FRAME", "base_link")
    )
...
def main() -> int:
    ...
    capture = RosRGBDFrameSource(
        ...
        sensor_model=f"{os.environ.get('A1Z_CAMERA_SOURCE', 'ros')}_d405",
